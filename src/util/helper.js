export function groupPurchasesByCategory(purchases) {
...
export function groupPurchasesByEvaluation(purchases) {
  if (!Array.isArray(purchases)) {
    throw new Error("Input should be an array");
  }

  const evaluationCounts = purchases.reduce((acc, purchase) => {
