...
import { groupPurchasesByEvaluation } from "../util/helper";
import { getAllPurchases } from "../apiService";
import { Purchase, GroupedDataEvaluation } from "../type";
...
function Evaluation() {
  ...
  const [groupedData, setGroupedData] = useState<GroupedDataEvaluation[]>([]);
  ...
  const navigation = groupedData.map((item) => ({
    ...
    count: item.count,
    current: currentTab === item.evaluation,
  }));

  const filteredPurchases = purchases.filter(
    (purchase) => purchase.Evaluation === currentTab
  );
  ...
  useEffect(() => {
    async function initialSetUp() {
      ...
      try {
        ...
        setPurchases(data);
        setGroupedData(groupPurchasesByEvaluation(data));
      } catch (error) {
        console.error(error);
      } finally {
        setIsLoading(false);
